...
void NES::PPU::fetchSprites() {
    uint8_t spriteHeight = reg.control.flags.TallSprites ? 16 : 8;
    spriteZeroIndex = 0xFF;
    std::fill_n(sprTiles, 0x10, 0x0);
    std::fill_n(sprXPositions, 0x8, 0x0);
    for (int i = 0; i < 8; i++) {
        sprAttributes[i] = SpriteAttributes{0x0, 0x0, 0x0, 0x0, 0x0};
    }
    
    for (int i = 0; i < activeSpriteCount; i++) {
        Sprite *sprite = (Sprite*)&oam[spr[i]];
        if (spr[i] == 0) spriteZeroIndex = i;
        ...
        sprXPositions[i] = sprite->xPosition;
    }
}
...
void NES::PPU::renderPixel() {
    ...
    if (renderSprites == true) {
        for (int i = 0; i < 8; i++) {
            uint8_t xPosition = sprXPositions[i];
            
            if (x >= xPosition &&
                x  < xPosition + 8) {
                SpriteAttributes attributes = sprAttributes[i];
                uint8_t spriteX = x - xPosition;
                ...
                reg.status.flags.Sprite0Hit |= (i == spriteZeroIndex && spriteColor != 0x0 && backgoundColor != 0x0 && currentCycle != 255);
...
void NES::PPU::step() {
    ...
    // Sprites
    if (visibleScanline && currentCycle == 65) evaluateSprites(); //TODO This should be happening at 65 for next scanline
    if (visibleScanline && currentCycle == 257) fetchSprites();
